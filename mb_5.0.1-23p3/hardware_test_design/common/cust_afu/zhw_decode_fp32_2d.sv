...
module decode_stream_sc_module_2 (
  ...
  integer dbg_x_reported_ds = 0; // 1-shot X detection flag
  integer dbg_fifo_rd_cnt = 0; // count FIFO reads for debug
  ...
  logic [0:0] bc_drain_hold; // BUG FIX #11: keep s_bc_ready=1 after drain until next header
  ...
  always_ff @(posedge clk) begin: ms_proc // FIX: changed from negedge to posedge to match SystemC timing
    ...
    if ((reset) == (0)) begin
      ...
      // BUG FIX #10: Skip section only fires in explicit skip mode (skpbts=1).
      // In SystemC, _s_blk_cycle entering the skip section is the header's else
      // branch setting skpbts=1. The original codegen merged _s_blk_cycle||skpbts
      // which caused the skip section to fire on the cycle after header processing
      // (due to NBA delay on s_blk_start_ready keeping _s_blk_cycle=1).
      if ((!(csync_scclang_global_4)) && (skpbts_scclang_global_6)) begin
        w_rembits_ms_proc_local_12 = c_rembits_scclang_global_2;
        if (0)
          $display("SKIP_DBG [%0t] c_rembits=%0d w_rembits=%0d skpbts=%b blk_cycle=%b c_wordoff=%0d",
            $time, c_rembits_scclang_global_2, w_rembits_ms_proc_local_12, skpbts_scclang_global_6, _s_blk_cycle_ms_proc_local_10, c_wordoff_scclang_global_3);
        if ((w_rembits_ms_proc_local_12) > (0)) begin
          if ((w_rembits_ms_proc_local_12) / (32)) begin
            ...
            // BUG FIX: SystemC uses c_wordoff+dreg_bits, not dreg_bits+1
            zhw__decode_streamfp_t8_23_bits_t32_2__get_window_func_3(b_wrk_ms_proc_local_8_f__ref_0,b_wrk_ms_proc_local_8_w__ref_0,c_wordoff_scclang_global_3 + dreg_bits_ms_proc_local_13,b_wrk_ms_proc_local_8_f__ref_0,b_wrk_ms_proc_local_8_w__ref_0);
          end
          ...
          if ((w_rembits_ms_proc_local_12) > (32)) begin
            ...
            // BUG FIX: SystemC uses (c_wordoff+dreg_bits)%pr_w, not (dreg_bits+1)%32
            c_wordoff_scclang_global_3 <= (c_wordoff_scclang_global_3 + dreg_bits_ms_proc_local_13) % (32);
            skpbts_scclang_global_6 <= 0;
          end
      ...
      // BUG FIX #4: Added c_rembits==0 guard (missing from codegen). In SystemC,
      // header processing only happens when previous block's bits are fully consumed.
      // If c_rembits > 0, we trigger skip_bits to drain remaining bits first.
      // BUG FIX #7: Removed (m_bp_ready)&&(s_bc_valid) guard from header section.
      // In SystemC, header processing is independent of the bit-plane handshake.
      // The codegen merged them causing circular deadlock: header needs bp_ready,
      // bp_ready needs pipeline drained, pipeline needs header from inv_cast.
      // Header section: fires on _s_blk_cycle (new block arrival) when not in skip mode.
      // BUG FIX #10: Guard with !s_blk_start_ready to prevent re-entry on the cycle
      // after header processing. On cycle N, header fires and sets s_blk_start_ready<=1
      // (NBA). On cycle N+1, s_blk_start_ready=1 (NBA took effect) but _s_blk_cycle
      // is still 1 because decode_ints hasn't deasserted s_blk_start_valid yet.
      // Without the guard, the header section fires again and erroneously triggers
      // skip mode (c_rembits>0 from the just-parsed header).
      if (((!(csync_scclang_global_4)) && (_s_blk_cycle_ms_proc_local_10) && (!s_blk_start_ready)) && (!(skpbts_scclang_global_6))) begin
        if (c_rembits_scclang_global_2 <= 0) begin
          // --- Header processing (zerobit flow A) ---
          bc_drain_hold <= 0; // clear drain hold — new block starting
          ...
          if (!(bhdr_ms_proc_local_11_zb__ref_0)) begin
            ...
            s_bc_ready <= 0;
            skpbts_scclang_global_6 <= 1;
            s_blk_start_ready <= 0; // FIX: SystemC does NOT ACK blk_start for zero blocks
          end

          c_wordoff_scclang_global_3 <= w_wordoff_ms_proc_local_9;
          c_rembits_scclang_global_2 <= w_rembits_ms_proc_local_12;
          $display("DS_DBG [%0t] writing bhdr: zb=%b exp=%02x  wordoff=%0d bc_data=%0d  b_c_f=%b%b%b%b  b_c_w0=0x%08x b_c_w1=0x%08x", $time, bhdr_ms_proc_local_11_zb__ref_0, bhdr_ms_proc_local_11_exp__ref_0, w_wordoff_ms_proc_local_9, s_bc_data, b_c_scclang_global_0_f[0], b_c_scclang_global_0_f[1], b_c_scclang_global_0_f[2], b_c_scclang_global_0_f[3], b_c_scclang_global_0_w[0], b_c_scclang_global_0_w[1]);
          m_bhdr_data_zb <= bhdr_ms_proc_local_11_zb__ref_0;
          m_bhdr_data_exp <= bhdr_ms_proc_local_11_exp__ref_0;
          m_bhdr_valid <= 1;
        end else begin
          // --- Previous block has remaining bits — trigger skip (SystemC else branch) ---
          bhdr_ms_proc_local_11_zb__ref_0 = 1;  // mark as zero (don't send downstream)
          m_bhdr_valid <= 0;
          m_bp_valid <= 0;
          s_bc_ready <= 0;
          skpbts_scclang_global_6 <= 1;
      ...
      // BUG FIX #10: Use registered m_bhdr_data_zb instead of bhdr_ms_proc_local_11_zb__ref_0.
      // The local bhdr_zb is re-initialized to 1 (zero-block) every cycle by init_func_2,
      // so !bhdr_zb is always 0 and the bit-plane section can never fire when _s_blk_cycle=1.
      // The registered m_bhdr_data_zb persists from header processing and correctly reflects
      // whether the current block is zero or non-zero.
      //
      // BUG FIX #11: Split the bit-plane section into two paths:
      //   (a) c_rembits > 0: accept bc, advance bitstream, send next plane (normal path)
      //   (b) c_rembits <= 0: accept bc only (drain) — no plane sent, no bitstream advance
      // Path (b) handles the last bc of a block: decode_ints needs its bc acknowledged
      // to complete the block, but decode_stream must NOT send another plane (the block's
      // bit budget is exhausted). Without path (b), the last bc is never accepted and
      // the pipeline deadlocks.
      // Bit-plane section fires when:
      // - !_s_blk_cycle: normal inter-block-start operation (most cycles)
      // - _s_blk_cycle && s_blk_start_ready: first-plane cycle right after header
      //   processing (header set s_blk_start_ready=1 via NBA on previous cycle).
      //   On this cycle, w_rembits/w_wordoff are from header, so !_s_blk_cycle guard
      //   inside correctly prevents bc subtraction (keeping header's values).
      // - NOT when _s_blk_cycle && !s_blk_start_ready: new block request just arrived,
      //   header hasn't processed yet. Bit-plane section must not fire here because it
      //   would send a duplicate/bogus plane and not update c_rembits.
      if ((((!(csync_scclang_global_4)) && ((!(_s_blk_cycle_ms_proc_local_10)) || ((s_blk_start_ready) && (!(m_bhdr_data_zb))))) && (!(skpbts_scclang_global_6))) && ((m_bp_ready) && (s_bc_valid))) begin
        if (c_rembits_scclang_global_2 > 0) begin
          // Path (a): Normal bit-plane exchange — accept bc, send next plane
          // BUG FIX: w_rembits update must be guarded by !_s_blk_cycle (SystemC source)
          // On block-start cycles, w_rembits retains value from header section above
          if (!(_s_blk_cycle_ms_proc_local_10)) begin
            w_rembits_ms_proc_local_12 = (c_rembits_scclang_global_2) - (s_bc_data);
            // BUG FIX #13: Cap wordoff advance to remaining budget. On the last plane,
            // decode_ints may consume more bits from the 32-bit window than remain in
            // the block (bc_data > c_rembits). The excess bits are past the block boundary.
            // Advancing by the full bc_data would misalign the bitstream for subsequent
            // blocks. Capping to c_rembits positions at the exact block boundary.
            if (s_bc_data > c_rembits_scclang_global_2)
              w_wordoff_ms_proc_local_9 = (w_wordoff_ms_proc_local_9) + (c_rembits_scclang_global_2);
            else
          ...
          w_wordoff_ms_proc_local_9 = (w_wordoff_ms_proc_local_9) % (32);
          if (1)
            $display("BP_DBG [%0t] bc=%0d woff=%0d rembits=%0d plane=0x%08x f=%b%b%b%b w0=0x%08x w1=0x%08x w2=0x%08x w3=0x%08x",
              $time, s_bc_data, w_wordoff_ms_proc_local_9, w_rembits_ms_proc_local_12,
              planewdw_ms_proc_local_15,
              b_wrk_ms_proc_local_8_f__ref_0[0], b_wrk_ms_proc_local_8_f__ref_0[1],
              b_wrk_ms_proc_local_8_f__ref_0[2], b_wrk_ms_proc_local_8_f__ref_0[3],
              b_wrk_ms_proc_local_8_w__ref_0[0], b_wrk_ms_proc_local_8_w__ref_0[1],
              b_wrk_ms_proc_local_8_w__ref_0[2], b_wrk_ms_proc_local_8_w__ref_0[3]);
          ...
          // Path (b): Drain — accept bc without sending new plane (block budget exhausted)
          //DRAIN_DBG disabled
          s_bc_ready <= 1;
          m_bp_valid <= 0;
          bc_drain_hold <= 1;
        ...
        if ((m_bp_ready) || (s_bc_valid)) begin
          m_bp_valid <= 0;
          // While bc_drain_hold is set, keep s_bc_ready=1 to prevent m_bc_valid oscillation
          if (!bc_drain_hold) begin
            s_bc_ready <= 0;
          end
        end
      end

      if (!dbg_x_reported_ds && $isunknown(c_m_bfifo_scclang_global_8_data_tdata) && c_m_bfifo_scclang_global_8_valid)
        $display("BFIFO_X [%0t] FIRST_X: valid=%b ready=%b dout=0x%08x b_c_f=%b%b%b%b b_c_w0=0x%08x",
          $time, c_m_bfifo_scclang_global_8_valid, c_m_bfifo_scclang_global_8_ready,
          c_m_bfifo_scclang_global_8_data_tdata,
          b_c_scclang_global_0_f[0], b_c_scclang_global_0_f[1], b_c_scclang_global_0_f[2], b_c_scclang_global_0_f[3],
          b_c_scclang_global_0_w[0]);
  ...
  begin
    ...
    for (i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_21 = 0;(i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_21) < ((32) / (32));i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_21++) begin
      w_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_18_f[i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_21] = 0;
      w_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_18_w[i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_21] = 0;
    end
    for (i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_22 = 0;(i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_22) < (4);i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_22++) begin
      tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_f[i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_22] = 0;
      tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_w[i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_22] = 0;
    end
    ...
    if (c_m_bfifo_scclang_global_8_valid) begin
      if (c_m_bfifo_scclang_global_8_ready) begin
        for (i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_24 = 0;(i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_24) < ((32) / (32));i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_24++) begin
          ...
          w_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_18_w[i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_24] = (word_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_17_tdata__ref_0) >>> ((32) * (i_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_24)); // FIX: use __ref_0 (post-constructor value) not the uninitialized pre-call copy
        end
      end
      ...
      // BUG FIX #19: Always store BFIFO word when valid && ready, even if buffer
      // appears full (tgtreg >= 3) after compaction. Due to NBA timing, ready=1
      // was set based on PREVIOUS cycle's buffer state; by the time the word arrives,
      // get_window may not have consumed a slot yet (NBA lag), making the buffer
      // appear full. Storing to slot 3 (the 4th slot) prevents word loss.
      // The buffer has 4 slots [0:3]; the original code only uses 3 (reg_thresh=3).
      if ((tgtreg_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_26) < (4)) begin
        if (c_m_bfifo_scclang_global_8_ready) begin
      ...
      // REFRESH_DBG: track FIFO word consumption — print when tgtreg >= 3 (potential overflow)
      if (c_m_bfifo_scclang_global_8_ready) begin
        dbg_fifo_rd_cnt = dbg_fifo_rd_cnt + 1;
        if (tgtreg_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_26 >= 3 || dbg_fifo_rd_cnt <= 10) begin
          $display("REFRESH_DBG [%0t] rd#%0d word=0x%08x srcreg=%0d tgtreg=%0d w_f=%b pb_f=%b%b%b%b tmp_f=%b%b%b%b",
            $time, dbg_fifo_rd_cnt,
            w_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_18_w[0],
            srcreg_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_25,
            tgtreg_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_26,
            w_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_18_f[0],
            pb_c_f[0], pb_c_f[1], pb_c_f[2], pb_c_f[3],
            tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_f[0],
            tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_f[1],
            tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_f[2],
            tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_f[3]);
        end
      end
    end else begin
    ...
    if (!dbg_x_reported_ds &&
        ($isunknown(tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_w[0]) ||
         $isunknown(tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_w[1]) ||
         $isunknown(tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_w[2]) ||
         $isunknown(tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_w[3]))) begin
      dbg_x_reported_ds = 1; // 1-shot: only report first X occurrence
      $display("REFRESH_X [%0t] FIRST_X: valid=%b ready=%b bfifo_tdata=0x%08x word_tdata_ref0=0x%08x w_f=%b w_w0=0x%08x tmp_f=%b%b%b%b tmp_w0=0x%08x tmp_w1=0x%08x pb_c_f=%b%b%b%b pb_c_w0=0x%08x pb_c_w1=0x%08x pb_c_w2=0x%08x pb_c_w3=0x%08x",
        $time, c_m_bfifo_scclang_global_8_valid, c_m_bfifo_scclang_global_8_ready,
        c_m_bfifo_scclang_global_8_data_tdata,
        word_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_17_tdata__ref_0,
        w_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_18_f[0],
        w_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_18_w[0],
        tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_f[0],
        tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_f[1],
        tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_f[2],
        tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_f[3],
        tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_w[0],
        tmp_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_20_w[1],
        pb_c_f[0], pb_c_f[1], pb_c_f[2], pb_c_f[3],
        pb_c_w[0], pb_c_w[1], pb_c_w[2], pb_c_w[3]);
    end
...
//       read_data_zhw__decode_streamfp_t8_23_bits_t32_2__refresh_next_bs_regs_func_7_local_23; // no-op
    end
  ...
  begin
    ...
    if (wordoff__local_28 >= 3)
      $display("GW_OOB [%0t] wordoff=%0d bitoff=%0d wb_c_f=%b%b%b%b wb_c_w0=0x%08x wb_c_w1=0x%08x wb_c_w2=0x%08x wb_c_w3=0x%08x",
        $time, wordoff__local_28, bitoff_actual_scclang_global_13,
        wb_c_f[0], wb_c_f[1], wb_c_f[2], wb_c_f[3],
        wb_c_w[0], wb_c_w[1], wb_c_w[2], wb_c_w[3]);
  ...
  begin
    ...
    if ((_MAX__local_36) < (0)) begin
      _MAX__local_36 = 0; // clamp negative to zero
    end
  ...
  begin
    // Copy constructor: word = bfifo_data
    hthis_tdata__ref_0 = NONAME_tdata;
    hthis_tlast__ref_0 = NONAME_tlast;
  end
  endfunction
endmodule
...
module fifo_cc_sc_module_10 (
  ...
  always_ff @(posedge clk) begin: ms_proc
    ...
    if ((reset) == (0)) begin
      ...
      // FWFT mode: combinatorial read in mc_proc, no registered read here
      ...
      if (rd_en_i_scclang_global_3) begin
        $display("BFIFO_RD [%0t] rd_idx=%0d->%0d data=0x%08x wr_idx=%0d empty_i=%b full_i=%b wr_en_i=%b",
          $time, rd_idx_scclang_global_4, rd_inc_ms_proc_local_4,
          data_scclang_global_0_tdata[rd_idx_scclang_global_4],
          wr_idx_scclang_global_6, empty_i_scclang_global_1, full_i_scclang_global_2, wr_en_i_scclang_global_5);
...
// END fifo_cc_sc_module_10 (BFIFO)
...
module fifo_cc_sc_module_11 (
  ...
  always_ff @(posedge clk) begin: ms_proc
    ...
    if ((reset) == (0)) begin
      ...
      // FWFT mode: combinatorial read in mc_proc
      if (wr_en_i_scclang_global_5) begin
...
module decode_ints_sc_module_4 (
  ...
  integer di_dbg_blk_cnt = 0;
  // BUG FIX #14: bp_consumed prevents decode_ints from processing the same
  // bp_data window twice. The bp/bc round-trip between decode_stream and
  // decode_ints takes 2 cycles (NBA delays). Without this flag, decode_ints
  // processes one plane per cycle but receives a new window every 2 cycles,
  // causing it to process two planes from the same window data. This shifts
  // the bitstream position by one bit per double-process, corrupting all
  // decoded values. The flag is SET after processing a plane and CLEARED
  // on the next cycle, creating a 2-cycle-per-plane cadence that matches
  ...
  always_ff @(posedge clk) begin: ms_proc
    ...
    if ((reset) == (0)) begin
      ...
      // BUG FIX #9: Use (m_ready || !m_valid) instead of just m_ready for handshake.
      // When m_valid=0 (no pending block output), the bit-plane handshake can run
      // even when bb1 is full. This prevents deadlock when short blocks fill bb1
      // faster than decode_block drains it. When m_valid=1, we still need m_ready
      // to prevent c_bplane overwrite by the next block's planes.
      // BUG FIX #12: Gate handshake with !m_blk_start_valid. When decode_ints has
      // asserted blk_start (requesting next block header from decode_stream), it must
      // NOT participate in the plane/bc handshake. Without this gate, stale s_bp_valid
      // causes m_bc_valid=1, and decode_stream accepts the bc, corrupting c_rembits.
      // BUG FIX #15: Gate handshake with !bp_consumed. When decode_ints has already
      // processed the current plane (bp_consumed=1), it must NOT assert m_bc_valid.
      // Without this gate, decode_stream sees valid bc on the "consumed" cycle and
      // advances the bitstream pointer a second time using the same stale bc_data,
      // causing a double-advance (e.g., woff jumps by 2*bc instead of bc). The gate
      // ensures DS only sees one valid bc per plane, matching the SystemC semantics
      // where DS blocks on the bc channel read until DI produces a new value.
      m_bc_valid_ms_proc_local_6 = (!bp_consumed) && (!m_blk_start_valid) && (m_ready || !m_valid) && ((!(m_bc_ready)) || (s_bp_valid));
      s_bp_ready_ms_proc_local_7 = (!bp_consumed) && (!m_blk_start_valid) && (m_ready || !m_valid) && ((!(s_bp_valid)) || (m_bc_ready));
      s_bp_ready <= s_bp_ready_ms_proc_local_7;
      m_bc_valid <= m_bc_valid_ms_proc_local_6;
      if ((m_blk_start_ready) == (1)) begin
        m_blk_start_data <= 0;
        m_blk_start_valid <= 0;
        // BUG FIX #17: Clear stale bc data when acknowledging block-start.
        // After the last plane of block N, m_bc_data retains the bc value from
        // that plane (e.g., bc=1 for k=21). When DI bootstraps the handshake
        // for block N+1 (asserting m_bc_valid before any plane is processed),
        // DS sees the stale s_bc_data and advances the bitstream by that many
        // bits, misaligning all subsequent planes. For block 0, m_bc_data=0
        // (from reset) so the bootstrap works correctly. For block N+1, we
        // must replicate the reset behavior by clearing m_bc_data to 0.
        m_bc_data <= 0;
      end
      // BUG FIX #9 (part 2): Also add !m_valid here so processing continues when
      // no output is pending, even if bb1 is full (m_ready=0) and k==kmin.
      // BUG FIX #12: Guard plane processing with !m_blk_start_valid. After the last
      // plane of a block (last=1), m_blk_start_valid is asserted via NBA. On the next
      // cycle, s_bp_valid may still be high (stale plane from decode_stream). Without
      // the guard, decode_ints processes this bogus plane and sets m_blk_start_valid<=0
      // (last=0), losing the block-start request for the next block. In SystemC,
      // event-driven semantics prevent this — the process sleeps until blk_start is acked.
      // BUG FIX #14: Default-clear bp_consumed so DI accepts next window after one wait cycle.
      // The processing block below overrides this with <= 1 when it fires.
      bp_consumed <= 0;
      // BUG FIX #14: Gate with !bp_consumed to process each window only once.
      if ((!bp_consumed) && (!m_blk_start_valid) && (((m_ready) || (!m_valid) || ((k_scclang_global_1) != (kmin_scclang_global_2))) && (m_bc_ready)) && (s_bp_valid)) begin
        ...
        if (di_dbg_blk_cnt == 124 || di_dbg_blk_cnt == 260 || di_dbg_blk_cnt == 339 || di_dbg_blk_cnt == 709 || di_dbg_blk_cnt == 719)
          $display("DI_IN  [%0t] blk=%0d k=%0d n=%0d bits=%0d m=%0d window=0x%08x x_init=0x%08x", $time, di_dbg_blk_cnt, k_scclang_global_1, n_scclang_global_3, bits_scclang_global_7, m_ms_proc_local_8, stream_window_scclang_global_6, x_scclang_global_5);
        // BUG FIX #20: SystemC source uses while(n < 16), but systemc-clang
        // translated it to for(i=0;i<16;i++) which is too few iterations.
        // The inner run-length loop can consume up to 14 iterations for zeros,
        // plus outer tests and re-entry decrements. Need ~48 for worst case.
        for (i_ms_proc_local_9 = 0;(i_ms_proc_local_9) < (48);i_ms_proc_local_9++) begin
        ...
        if (di_dbg_blk_cnt == 124 || di_dbg_blk_cnt == 260 || di_dbg_blk_cnt == 339 || di_dbg_blk_cnt == 709 || di_dbg_blk_cnt == 719)
          $display("DI_OUT [%0t] blk=%0d k=%0d n=%0d bits=%0d x=0x%04x bc=%0d", $time, di_dbg_blk_cnt, k_scclang_global_1, n_scclang_global_3, bits_scclang_global_7, x_scclang_global_5[15:0], bitoff_scclang_global_8);
        last_ms_proc_local_11 = ((bits_scclang_global_7) == (0)) || ((k_scclang_global_1) <= (kmin_scclang_global_2));
        if (last_ms_proc_local_11) begin
          k_scclang_global_1 <= (32) - (1);
          n_scclang_global_3 = 0;
          bits_scclang_global_7 = (s_maxbits) - ((8) + (1));
          di_dbg_blk_cnt = di_dbg_blk_cnt + 1;
        end else begin
          k_scclang_global_1 <= (k_scclang_global_1) - (1);
        end

        bp_consumed <= 1; // BUG FIX #14: mark window as consumed
        m_valid <= last_ms_proc_local_11;
        m_blk_start_valid <= last_ms_proc_local_11;
        m_blk_start_data <= last_ms_proc_local_11;
        if (last_ms_proc_local_11) begin
          $display("DI_BPLANE [%0t] k=%0d kmin=%0d planes[31:16]: %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x",
            $time, k_scclang_global_1, kmin_scclang_global_2,
            c_bplane_scclang_global_0[31], c_bplane_scclang_global_0[30],
            c_bplane_scclang_global_0[29], c_bplane_scclang_global_0[28],
            c_bplane_scclang_global_0[27], c_bplane_scclang_global_0[26],
            c_bplane_scclang_global_0[25], c_bplane_scclang_global_0[24],
            c_bplane_scclang_global_0[23], c_bplane_scclang_global_0[22],
            c_bplane_scclang_global_0[21], c_bplane_scclang_global_0[20],
            c_bplane_scclang_global_0[19], c_bplane_scclang_global_0[18],
            c_bplane_scclang_global_0[17], c_bplane_scclang_global_0[16]);
          $display("DI_BPLANE [%0t] planes[15:0]: %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x %04x  last_bp[%0d]=%04x",
            $time,
            c_bplane_scclang_global_0[15], c_bplane_scclang_global_0[14],
            c_bplane_scclang_global_0[13], c_bplane_scclang_global_0[12],
            c_bplane_scclang_global_0[11], c_bplane_scclang_global_0[10],
            c_bplane_scclang_global_0[9], c_bplane_scclang_global_0[8],
            c_bplane_scclang_global_0[7], c_bplane_scclang_global_0[6],
            c_bplane_scclang_global_0[5], c_bplane_scclang_global_0[4],
            c_bplane_scclang_global_0[3], c_bplane_scclang_global_0[2],
            c_bplane_scclang_global_0[1], c_bplane_scclang_global_0[0],
            k_scclang_global_1, x_scclang_global_5);
        end
      end
...
module inv_cast_sc_module_8 (
  ...
  integer ic_blk_num = 0; // debug block counter
  ...
  always_ff @(posedge clk) begin: ms_proc
    ...
    if ((reset) == (0)) begin
      ...
      if ((((count_scclang_global_1) == (0)) && (s_bhdr_valid)) && ((got_bhdr_scclang_global_2) == (0))) begin
        $display("IC_DBG [%0t] accepting hdr: s_bhdr_data_zb=%b s_bhdr_data_exp=%02x s_bhdr_valid=%b s_valid=%b", $time, s_bhdr_data_zb, s_bhdr_data_exp, s_bhdr_valid, s_valid);
        ...
        if (_zb_ms_proc_local_13) begin
          ...
          // BUG FIX #8: Do NOT set s_ready for zero blocks. SystemC does not.
          // The prior "s_ready <= s_valid" was wrong — it consumed bb2 data
          // meant for the next non-zero block, causing header/data desync.
        end
        s_bhdr_ready <= 1;
      end else begin
        s_bhdr_ready <= 0;
      ...
      if (((count_scclang_global_1) == (0)) && (((_got_bhdr_ms_proc_local_14) && (!(_zb_ms_proc_local_13))) || ((got_bhdr_scclang_global_2) && (!(zb_scclang_global_6))))) begin
        if (s_valid) begin
          ...
          s_ready <= 1;
          // Only count/print on first load (got_blk was 0 → transitioning to 1)
          if (!got_blk_scclang_global_3) begin
            if (ic_blk_num < 12) begin
              $display("IC_LOAD [%0t] blk=%0d: %08x %08x %08x %08x  %08x %08x %08x %08x  %08x %08x %08x %08x  %08x %08x %08x %08x",
                $time, ic_blk_num,
                s_block[0],  s_block[1],  s_block[2],  s_block[3],
                s_block[4],  s_block[5],  s_block[6],  s_block[7],
                s_block[8],  s_block[9],  s_block[10], s_block[11],
                s_block[12], s_block[13], s_block[14], s_block[15]);
            end
            ic_blk_num = ic_blk_num + 1;
          end
        end
      end
      if (got_blk_scclang_global_3) begin
      ...
      if (c_sync_scclang_global_0) begin
        if (ic_blk_num <= 12) begin
          $display("IC_OUT  [%0t] blk=%0d pos=%0d r_blk=%08x",
            $time, ic_blk_num-1, count_scclang_global_1, r_blk_scclang_global_4[count_scclang_global_1]);
        end
  ...
  begin
    ...
    { { hthis_sign__ref_0, hthis_expo__ref_0 }, hthis_frac__ref_0 } = ui_actual_scclang_global_7;
  end
  endfunction
endmodule
